...
retained ADS provenance snapshot, and one canonical set of final figure
outputs.
...
- `code/001a-update-ads-records.ipynb` mirrors the collection logic in
  `001` and can be used to append a new year to an existing ADS snapshot
...
- `004-build-lexical-data.ipynb` writes the canonical yearly unigram
  parquet used by the lexical figure.
- `005-build-paper-assets.R` rebuilds the final paper figures and table
  from the canonical parquets only.

## Canonical directories

- `code/`: notebook-led upstream pipeline and shared helpers
- `code/stage-outputs/`: stage-local caches, audit tables, and support
  exports that are not part of the canonical shared data contract. In
  the public repo, only the retained stage-001 ADS snapshot archive is
  kept there by default.
- `data/`: local target directory for canonical machine-readable
  intermediates rebuilt from the retained ADS snapshot
...
- The manuscript source, manuscript PDF, and bibliography are
  intentionally kept out of the public repository surface.
...
If you need to rebuild from the ADS outputs, run the full canonical
pipeline in order.

Before running `code/002-build-canonical-data.R` from the retained
stage-001 archive, extract:
